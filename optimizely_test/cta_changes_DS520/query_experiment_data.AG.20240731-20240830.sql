...
-- template vars: AG, 2712637656, 20240731, 20240830
--
with variant_list as
(SELECT DISTINCT
    user_pseudo_id userid
        ,CASE WHEN event_params:cd81 LIKE '%797659%' THEN 'Control'
         WHEN event_params:cd81 LIKE '%797660%' THEN 'Variant' end as Variant
    FROM EDW.WEB_ACTIVITY.GA4_EVENT
    WHERE stream_id=2712637656 and (event_date between '20240731' and '20240830')
           AND CONTAINS(event_params:cd80, 'ag_tour_headline_copy' ))
...
INNER JOIN no_multiples
    ...
    WHERE stream_id=2712637656 and (event_date between '20240731' and '20240830')
    --AND event_name='page_view'  -- should also restrict to 'SRP': event_params:cd10 in ('pdp', 'pdp_map') ???
