...
-- template vars: Rent, 2119933141, 20240807, 20240820
--
with variant_list as
(SELECT DISTINCT
    user_pseudo_id userid
        ,CASE WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_control%' THEN 'Control'
         WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_v1%' THEN 'Variant1' 
         WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_v2%' THEN 'Variant2'
         end as Variant
         
    FROM EDW.WEB_ACTIVITY.GA4_EVENT
    WHERE stream_id=2119933141 and (event_date between '20240807' and '20240820')
           AND event_params:cd80 LIKE '%3d_tour_thumbnails_test%')
...
, session_check2 AS (
    SELECT 
        concat(user_pseudo_id, '_', event_params:ga_session_id) AS sessionid,
        user_pseudo_id AS userid,
        CASE 
            WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_control%' THEN 'Control'
            WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_v1%' THEN 'Variant1'
            WHEN event_params:cd81 LIKE '%3d_tour_thumbnails_v2%' THEN 'Variant2'
        END AS Variant,
        MIN(event_timestamp) AS first_hit  -- First hit timestamp
    FROM EDW.WEB_ACTIVITY.GA4_EVENT evt
    INNER JOIN no_multiples
        ON evt.user_pseudo_id = no_multiples.userid
    WHERE stream_id = 2119933141
      AND (event_date BETWEEN '20240807' AND '20240820')
      AND (event_name = 'page_view' AND event_params:cd10 LIKE '%pdp%')
      AND event_params:cd80 LIKE '%3d_tour_thumbnails_test%'
    GROUP BY 1, 2, 3
),
SessionExits AS (
    SELECT 
        concat(user_pseudo_id, '_', event_params:ga_session_id) AS sessionid,
        ROW_NUMBER() OVER (PARTITION BY concat(user_pseudo_id, '_', event_params:ga_session_id) ORDER BY event_timestamp DESC) AS event_rank,
        event_name,
        event_params:cd10
    FROM EDW.WEB_ACTIVITY.GA4_EVENT evt
    INNER JOIN no_multiples
    ...
    WHERE stream_id = 2119933141
      AND (event_date BETWEEN '20240807' AND '20240820')
      AND (event_name = 'page_view' AND (event_params:cd10 LIKE '%pdp%' OR event_params:cd10 LIKE '%srp%'))
      AND event_params:cd80 LIKE '%rent_new_highlights%'
),
session_check AS (
    SELECT 
        sc.sessionid,
        ...
        sc.Variant,
        sc.first_hit,
        COUNT(se.sessionid) AS total_exits_on_pdp  -- Total exits on PDP per session
    FROM session_check2 sc
    LEFT JOIN (
        SELECT 
            sessionid
        FROM SessionExits
        WHERE event_rank = 1  -- Only consider the last event in each session
          
    ) se ON sc.sessionid = se.sessionid
    GROUP BY sc.sessionid, sc.userid, sc.Variant, sc.first_hit
)
,session_info AS
(SELECT concat(user_pseudo_id, '_', event_params:ga_session_id) as sessionid
        ...
        , CASE WHEN SUM(total_exits_on_pdp) >=1 then 1 ELSE 0 END as exits,
    FROM EDW.WEB_ACTIVITY.GA4_EVENT evt
...
(SELECT concat(user_pseudo_id, '_', event_params:ga_session_id) as sessionid
    ...
    FROM EDW.WEB_ACTIVITY.GA4_EVENT evt
    WHERE stream_id=2119933141 and (event_date between '20240807' and '20240820')
    and event_params:cd10 LIKE '%pdp%'
...
WHERE stream_id=2119933141 and (event_date between '20240807' and '20240820')
    and event_params:cd10 LIKE '%pdp%'
    and event_params:cd13 = 'city'  -- XXX is this needed, given that 99+% pdps are 'city'
...
,COUNT(DISTINCT CASE WHEN event_params:cd67 IN ('phone', 'email') and event_params:cd10 LIKE '%pdp%' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as total_leads_srp
...
,count(distinct case when  event_params:cd130='phone' then event_params:cd09 end) as LT_phone
,count(distinct case when  event_params:cd130='email_no_intent' then event_params:cd09 end) as LT_email_no_intent
,count(distinct case when event_params:cd130='email_intent' then event_params:cd09 end) as LT_email_intent
,count(distinct case when  event_params:cd130='request_tour' then event_params:cd09 end) as LT_request_tour
,count(distinct case when event_params:cd130='schedule_tour' then event_params:cd09 end) as LT_schedule_tour
...
,COUNT(CASE WHEN lower(event_params:cd10) LIKE '%pdp%' and event_name='custom_click' and event_params:click_sub_type = 'pinpoint' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as map_pin_click
,COUNT(CASE WHEN lower(event_params:cd10) LIKE '%pdp%' and event_name='custom_click' and event_params:click_sub_type = 'favorite_properties' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as srp_saves   -- "cd41: save"; "cd41: unsave"
,COUNT(CASE WHEN lower(event_params:cd10) LIKE '%pdp%' and event_name='custom_click' and event_params:click_sub_type = 'share_button' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as srp_shares   -- "cd41: save"; "cd41: unsave"
,COUNT(CASE WHEN lower(event_params:cd10) LIKE '%pdp%' and event_name='custom_click' and event_params:click_type LIKE '%photo%gallery%' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as srp_gallery
,COUNT(CASE WHEN lower(event_params:cd10) LIKE '%pdp%' and event_name='custom_click' and event_params:click_sub_type LIKE '%3d_tour%' THEN concat(event_params:cd09, user_pseudo_id) ELSE null END) as threed_tour_click
FROM EDW.WEB_ACTIVITY.GA4_EVENT evt
LEFT JOIN session_check on session_check.sessionid = concat(user_pseudo_id, '_', event_params:ga_session_id)
WHERE stream_id=2119933141 and (event_date between '20240807' and '20240820')
    AND session_check.sessionid IS NOT NULL
...
, (length(test_renter_journey) - LENGTH(REGEXP_REPLACE(session_info.test_renter_journey, 'pdp', '')))/3 as srp_views
...
,  prop_interactions.threed_tour_click
FROM session_check
...
select *
,'Rent' as Brand
,coalesce(srp_pdp,0)+coalesce(map_pin_click,0)+coalesce(total_leads_srp,0)+coalesce(srp_saves,0)+coalesce(srp_shares,0)+coalesce(srp_gallery,0) as srp_engagements
