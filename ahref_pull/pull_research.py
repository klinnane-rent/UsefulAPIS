...
def initial_pull(domain, request, path_append=None, send_it=False):
    ...
    if send_it:
        ...
        write_json(fp.replace("/", "_"), response.json())
        #format_jsons(domain)
    else:   
        request = base_request.format(domain)
        response = requests.get(request)
        ...
        #print(response.json())
        print(response.json())
        append_to_json(response.json(), fp   )
...
def getNewLost(domains, retrieve_historics):
    path_append = "new_lost_research"
    date_after_equal = "2023-10-03"
    for domain in domains:
        #domain = domain.replace("/", "%2F")
        if retrieve_historics:
            request = "https://apiv2.ahrefs.com?token=5a94c007693f438ddf4bcd41c4f304ece2df0b8c&target={}&limit=1000000000&output=json&from=refdomains_new_lost&mode=subdomains"
            initial_pull( domain, request, path_append, retrieve_historics)
        else:
            request = "https://apiv2.ahrefs.com?token=5a94c007693f438ddf4bcd41c4f304ece2df0b8c&target={}&limit=1000000000&output=json&from=refdomains_new_lost&mode=subdomains&where=date%3E%3D%22{}%22".format(domain, date_after_equal)
            initial_pull( domain, request, path_append, retrieve_historics)
